...
import { equipmentQuery, type EquipmentWithProcesses } from "~/lib/api";
...
} from "~/components/mfg/equipment/EquipmentFilters";
...
export function EquipmentClient() {
  ...
  const [selectedEquipment, setSelectedEquipment] =
    useState<EquipmentWithProcesses | null>(null);
  ...
  const { data: equipment = [], isLoading, error } = useQuery(equipmentQuery());
  ...
  const handleEdit = (equipment: EquipmentWithProcesses) => {
    setSelectedEquipment(equipment);
    setEditDialogOpen(true);
  };

  const handleDelete = (equipment: EquipmentWithProcesses) => {
    setSelectedEquipment(equipment);
    setDeleteDialogOpen(true);
  };

  const handleViewDetails = (equipment: EquipmentWithProcesses) => {
    setSelectedEquipment(equipment);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="bg-background shrink-0 border-b px-4 py-4 sm:px-6 lg:px-8">
